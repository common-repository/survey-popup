 ...
 *
 */
if ( ! class_exists( 'SRVY_Field_link_color' ) ) {
  class SRVY_Field_link_color extends SRVY_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'srvy' ),
        'hover'    => esc_html__( 'Hover', 'srvy' ),
        'active'   => esc_html__( 'Active', 'srvy' ),
        'visited'  => esc_html__( 'Visited', 'srvy' ),
        'focus'    => esc_html__( 'Focus', 'srvy' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="srvy--left srvy-field-color">';
          echo '<div class="srvy--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="srvy-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
