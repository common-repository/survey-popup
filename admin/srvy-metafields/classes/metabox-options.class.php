 ...
 *
 */
if ( ! class_exists( 'SRVY_Metabox' ) ) {
  class SRVY_Metabox extends SRVY_Abstract{
    ...
    public function add_metabox_classes( $classes ) {
      ...
      if ( ! empty( $this->post_formats ) ) {
        ...
        foreach ( $this->post_formats as $format ) {
          $classes[] = 'srvy-post-format-'. $format;
        }

        if ( ! in_array( $saved_post_format, $this->post_formats ) ) {
          $classes[] = 'srvy-metabox-hide';
        } else {
          $classes[] = 'srvy-metabox-show';
        }
      ...
      if ( ! empty( $this->page_templates ) ) {
        ...
        $classes[] = 'srvy-page-templates';

        foreach ( $this->page_templates as $template ) {
          $classes[] = 'srvy-page-'. preg_replace( '/[^a-zA-Z0-9]+/', '-', strtolower( $template ) );
        }
    ...
    public function add_meta_box_content( $post, $callback ) {
      ...
      $errors    = ( is_object ( $post ) ) ? get_post_meta( $post->ID, '_srvy_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $theme     = ( $this->args['theme'] ) ? ' srvy-theme-'. $this->args['theme'] : '';
      $nav_type  = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object ( $post ) && ! empty( $errors ) ) {
        delete_post_meta( $post->ID, '_srvy_errors_'. $this->unique );
      }

      wp_nonce_field( 'srvy_metabox_nonce', 'srvy_metabox_nonce'. $this->unique );

      echo '<div class="srvy srvy-metabox'. esc_attr( $theme ) .'">';

        echo '<div class="srvy-wrapper'. esc_attr( $show_all ) .'">';

          if ( $has_nav ) {

            echo '<div class="srvy-nav srvy-nav-'. esc_attr( $nav_type ) .' srvy-nav-metabox">';
              ...
              foreach ( $this->sections as $section ) {
                ...
                $tab_error = ( ! empty( $errors['sections'][$tab_key] ) ) ? '<i class="srvy-label-error srvy-error">!</i>' : '';
                $tab_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="srvy-tab-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
          ...
          echo '<div class="srvy-content">';
            ...
            foreach ( $this->sections as $section ) {
              ...
              echo '<div class="srvy-section hidden'. esc_attr( $section_onload . $section_class ) .'">';

              echo ( $section_title || $section_icon ) ? '<div class="srvy-section-title"><h3>'. $section_icon . $section_title .'</h3></div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  SRVY::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
            ...
            if ( ! empty( $this->args['show_restore'] ) || ! empty( $this->args['show_reset'] ) ) {
              ...
              echo '<div class="srvy-sections-reset">';
              echo '<label>';
              echo '<input type="checkbox" name="'. esc_attr( $this->unique ) .'[_reset]" />';
              echo '<span class="button srvy-button-reset">'. esc_html__( 'Reset', 'srvy' ) .'</span>';
              echo '<span class="button srvy-button-cancel">'. sprintf( '<small>( %s )</small> %s', esc_html__( 'update post', 'srvy' ), esc_html__( 'Cancel', 'srvy' ) ) .'</span>';
              echo '</label>';
              echo '</div>';
    ...
    public function save_meta_box( $post_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'srvy_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'srvy_metabox_nonce' ) ) {
        return $post_id;
      }
      ...
      $data = apply_filters( "srvy_{$this->unique}_save", $data, $post_id, $this );

      do_action( "srvy_{$this->unique}_save_before", $data, $post_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $post_id, '_srvy_errors_'. $this->unique, $errors );
        }

      }

      do_action( "srvy_{$this->unique}_saved", $data, $post_id, $this );

      do_action( "srvy_{$this->unique}_save_after", $data, $post_id, $this );
